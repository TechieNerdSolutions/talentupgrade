...
<form action="<?php echo site_url('user/quiz_questions/'.$param3.'/edit/'.$param2); ?>" method="post" id = 'mcq_form'>
    ...
    <?php for ($i = 0; $i < $question_details['number_of_options']; $i++):?>
        <div class="form-group options">
            ...
            <div class="input-group">
                <input type="text" class="form-control" name = "options[]" id="option_<?php echo $i; ?>" placeholder="<?php echo translate('option_').$i; ?>" required value="<?php echo $options[$i]; ?>">
    ...
    <?php endfor;?>
		...
		<div class="form-group">
			<button type="submit" data-dismiss="modal" id = "submitButton" name="button" class="btn btn-block btn-info btn-rounded btn-sm "><i class="fa fa-plus"></i>&nbsp;<?php echo translate('save');?></button>
		</div>
...
$('#submitButton').click( function(event) {
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response == 1) {
                success_notify('<?php echo translate('question_has_been_updated'); ?>');
            }else {
                error_notify('<?php echo translate('no_options_can_be_blank_and_there_has_to_be_atleast_one_answer'); ?>');
            }
        }
    });
