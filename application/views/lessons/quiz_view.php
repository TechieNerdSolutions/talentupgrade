...
<div id="quiz-body">
    <div class="" id="quiz-header">
        <?php echo translate("quiz_title"); ?> : <strong><?php echo $lesson_details['title']; ?></strong><br>
        <?php echo translate("number_of_questions"); ?> : <strong><?php echo count($quiz_questions->result_array()); ?></strong><br>
        <?php if (count($quiz_questions->result_array()) > 0): ?>
            <button type="button" name="button" class="btn btn-sign-up mt-2" style="color: #fff;" onclick="getStarted(1)"><?php echo translate("get_started"); ?></button>
        <?php endif; ?>
    </div>
        ...
        <?php if (count($quiz_questions->result_array()) > 0): ?>
            ...
            ?>
                ...
                <div class="hidden" id = "question-number-<?php echo $key+1; ?>">
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <div class="card text-left quiz-card">
                                <div class="card-body">
                                    <h6 class="card-title"><?php echo translate("question").' '.($key+1); ?> : <strong><?php echo $quiz_question['title']; ?></strong></h6>
                                </div>
                    ...
                    <button type="button" name="button" class="btn btn-sign-up mt-2 mb-2" id = "next-btn-<?php echo $quiz_question['id'];?>" style="color: #fff;" <?php if(count($quiz_questions->result_array()) == $key+1):?>onclick="submitQuiz()"<?php else: ?>onclick="showNextQuestion('<?php echo $key+2; ?>')"<?php endif; ?> disabled><?php echo count($quiz_questions->result_array()) == $key+1 ? translate("check_result") : translate("submit_and_next"); ?></button>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </form>
